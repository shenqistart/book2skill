...
本文档记录本仓库后续批量蒸馏的固定循环。当前 goal 已建立，本轮目标分类为 `07_股权分配和融资`；通过审核后继续读取下一个未完成主题分类。
...
- `07_股权分配和融资`: 本轮执行中，目标是完成 7 本可用源文本、8 个分类级 skills、书籍关系 taxonomy、验证与推送。
- `08_团队建设`: 已存在分类级入口与目录，本轮不改动其内容。
- 下一批主题: `07_股权分配和融资` 审核通过后，从本地产品主题目录继续读取下一个未完成分类并重复本循环。
